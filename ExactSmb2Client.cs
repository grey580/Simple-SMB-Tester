...
{
    public enum ExactSmbDialectFamily
    {
        Smb2Only,
        Smb3Only
    }

    public class ExactSmb2Client : ISMBClient
    {
        ...
        private ExactSmbDialectFamily m_requestedDialectFamily;
        ...
        public ExactSmb2Client(ExactSmbDialectFamily requestedDialectFamily) : this(requestedDialectFamily, DefaultResponseTimeoutInMilliseconds)
        {
        }

        public ExactSmb2Client(ExactSmbDialectFamily requestedDialectFamily, int responseTimeoutInMilliseconds) : this(requestedDialectFamily, responseTimeoutInMilliseconds, false)
        {
        }

        public ExactSmb2Client(ExactSmbDialectFamily requestedDialectFamily, bool enableSMB311Support) : this(requestedDialectFamily, DefaultResponseTimeoutInMilliseconds, enableSMB311Support)
        {
        }

        public ExactSmb2Client(ExactSmbDialectFamily requestedDialectFamily, int responseTimeoutInMilliseconds, bool enableSMB311Support)
        {
            m_requestedDialectFamily = requestedDialectFamily;
        ...
        private bool NegotiateDialect()
        {
            ...
            if (m_requestedDialectFamily == ExactSmbDialectFamily.Smb2Only)
            {
        ...
        public ISMBFileStore TreeConnect(string shareName, out NTStatus status)
        {
            ...
            if (response != null)
            {
                ...
                if (response.Header.Status == NTStatus.STATUS_SUCCESS && response is TreeConnectResponse treeConnectResponse)
                {
                    ...
                    return new ExactSmb2FileStore(this, response.Header.TreeID, m_encryptSessionData || encryptShareData);
                }
            }
            else
            {
